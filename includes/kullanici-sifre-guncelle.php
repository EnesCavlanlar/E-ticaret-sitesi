...
try {
    $stmt = $conn->prepare("SELECT kullanici_sifre FROM kullanici WHERE kullanici_id = :id");
    $stmt->bindParam(':id', $_SESSION['kullanici_id']);
    $stmt->execute();
    $user = $stmt->fetch();

    if (!$user || $user['kullanici_sifre'] !== $data['current_password']) {
    ...
    $stmt = $conn->prepare("UPDATE kullanici SET kullanici_sifre = :yeni_sifre WHERE kullanici_id = :id");
